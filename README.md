...
[![Colab](https://img.shields.io/badge/Open_in-Colab-F9AB00?style=for-the-badge&logo=googlecolab&logoColor=black)](https://colab.research.google.com/)
...
ASTRA/
│
├── 📓 ASTRA_Stage2A_Colab.ipynb      ← Main notebook (run in Colab)
...
│                      │                                              │
│                      ▼                                              │
│     Stage 5: Task-Object Knowledge Graph (GNN / Weighted Graph)     │
│                      │                                              │
│                      ▼                                              │
...
At the heart of ASTRA is a **weighted additive scoring formula** that combines four complementary signals:
...
| **Sg** | Graph Relevance Score | **β = 0.40** | Knowledge graph edge weight for task→object relationship |
| **Ss** | Semantic Similarity | **γ = 0.25** | CLIP cosine similarity between task embedding Te and object embedding Ei, normalised from [−1,1] → [0,1] |
| **Sp** | Scene Proximity Score | **δ = 0.15** | Co-presence reinforcement + fallback rank boost from SceneContextReasoner ∈ [0, 1] |

**Weight constraint:** `α + β + γ + δ = 0.20 + 0.40 + 0.25 + 0.15 = 1.00`

**Final selection:** `O* = argmax Sf` over all detected objects

> **Why β is highest:** The knowledge graph encodes human-validated task-object affordances — it is the most task-specific signal available and deserves the most influence.
...
# Returns: [{'label': 'wine glass', 'confidence': 0.91, 'bbox': [x1,y1,x2,y2], 'crop': <ndarray>}, ...]
```

- Model: **YOLOv5s** (COCO-80 pretrained, ~14 MB)
...
# Semantic similarity
Ss = cosine_similarity(Te, Ei)   # ∈ [−1, 1]
```
...
### `SceneContextReasoner` — Scene Proximity (Sp)

Computes `Sp ∈ [0, 1]` for each object using two sub-components:
...
# → Sp = 0.40 (fallback boost: wine glass missing, cup is rank-2 alternative)
#         + 0.30 (co-presence: dining table + fork + bottle present)
# → Sp = min(1.0, 0.70)
```
...
O_star = ranked[0]
# → {'label': 'wine glass', 'final_score': 0.8741, 'confidence': 0.91,
#    'graph_score': 1.0, 'semantic_sim': 0.83, 'scene_score': 0.50, ...}
```
...
1. Open `ASTRA_Stage2A_SingleFile.py` in Colab  
...
Evaluated on the **COCO-Tasks benchmark** (Sawatzky et al., 2019) — 14 tasks, one image per task, CPU-only inference.
...
> *Results shown are from the software pipeline (Stage 2A — CPU inference). Actual scores vary slightly by image.*
...
Stage 1 ✅   Proposal & Architecture Design
             Knowledge graph construction · System specification

Stage 2A ✅  Software Pipeline (this repo)
             CPU-based Python implementation · 14-task evaluation
...
| **Output** | Object label + bounding box + Sf score + score breakdown |
...
| # | Citation |
|---|---------|
| [1] | **J. Xie et al.**, *"What Object Should I Use? — Task Driven Object Detection,"* arXiv:1904.03000, 2019. **[Core benchmark paper — defines the 14 tasks]** |
...
> All code, architecture, and extended scoring formula (`Sf = α·Ci + β·Sg + γ·Ss + δ·Sp`) are original work by Team VILSCORE (#389).
